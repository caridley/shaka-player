...
shaka.text.UITextDisplayer = class {
  ...
  constructor(video, videoContainer) {
    ...
    this.textContainer_.style.alignItems = 'flex-start';

    // Set the captions at the bottom by default.
    this.textContainer_.style.justifyContent = 'flex-start';
  ...
  setCaptionStyles_(cueElement, cue, parents, hasWrapper) {
    ...
    if (cue.firstNonEmptyCol > 0) {
      style.left = `${(cue.firstNonEmptyCol + 1) * 3.125}%`;
    ...
    // TODO: Implement line alignment with line number for negative numbers.
    ...
    if (cue.line) {
      ...
      if ( cue.lineInterpretation == Cue.lineInterpretation.LINE_NUMBER) {
        style.position = 'relative';
        if (cue.line >= 0) {
          // line is 'row' and it starts at 0.
          style.top = ((cue.line-1) / cue.cellResolution.rows) * 100 + '%';
        }
      }
