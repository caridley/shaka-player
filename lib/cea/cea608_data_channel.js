...
shaka.cea.Cea608DataChannel = class {
  ...
  controlPac_(b1, b2) {
    ...
    if (b2 - startingNumMap[row] > 0) {
      const indents = (Math.floor((b2 - startingNumMap[row])/2)) * 4;
      buf.moveCursor(indents);
    }
  ...
  /**
   * Handles TO - Tab Offset @num Column.
   * @private
   */
  controlTo_(num) {
    this.curbuf_.moveCursor(num - 32);
  }

  /**
   * Handles DER - Tab Offset @num Column.
   * @private
   */
  controlDer_() {
    this.curbuf_.deleteToEndOfRow();
  }
  ...
  isMiscellaneous_(b1, b2) {
    ...
    // b1 -> |0|0|0|1|C|1|0|F|
    // b2 -> |0|0|1|0|X|X|X|X|
    const normalMisc = (((b1 & 0xf6) === 0x14) && ((b2 & 0xf0) === 0x20));
    const tabOverMisc = ((b1 === 0x17) || (b1 === 0x1F)) &&
      ((b2 <= 0x23) && (b2 >= 0x20));
    return (normalMisc || tabOverMisc);
  }
