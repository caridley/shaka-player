...
shaka.cea.Cea608Memory = class {
   ...
   */
  deleteToEndOfRow() {
    for (let i = this.position; i < shaka.cea.Cea608Memory.CC_CHARS; i++) {
      this.rows_[this.row_][i] = new shaka.cea.CeaUtils.StyledChar(
          '', this.underline_, this.italics_,
  ...
  /**
   * Move the cursor to @num column.
   */
  moveCursor(num) {
    const newPos = this.position + num;
    if (num > 1) {
      for (let i = this.position; i < newPos; i++) {
        this.rows_[this.row_][i] = new shaka.cea.CeaUtils.StyledChar(
            '', this.underline_, this.italics_,
            this.backgroundColor_, this.textColor_);
      }
  ...
  setCursor(newPos) {
    ...
    if (this.position < 0) { // Negative cursor position
      this.position = 0;
    } else if (this.position > shaka.cea.Cea608Memory.CC_CHARS) {
      this.position = shaka.cea.Cea608Memory.CC_CHARS;
    }
  }
  ...
  addChar(set, b) {
    ...
    if (char && this.position < shaka.cea.Cea608Memory.CC_CHARS) {
      const styledChar = new shaka.cea.CeaUtils.StyledChar(
          char, this.underline_, this.italics_,
          this.backgroundColor_, this.textColor_);
      this.rows_[this.row_][this.position] = styledChar;
  ...
  eraseChar() {
    this.setCursor(this.position - 1);
    this.rows_[this.row_][this.position] = new shaka.cea.CeaUtils.StyledChar(
        '', this.underline_, this.italics_,
        this.backgroundColor_, this.textColor_);
  }
...
/**
 * Maximum character of row in the buffer.
 * @const {!number}
 */
shaka.cea.Cea608Memory.CC_CHARS = 32;
