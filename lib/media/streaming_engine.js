...
shaka.media.StreamingEngine = class {
  ...
  constructor(manifest, playerInterface) {
     ...
     */
    this.timestampOffsetCorrector_ = new shaka.media.TimestampOffsetCorrector();
  }
  ...
  async fetchAndAppend_(mediaState, presentationTime, reference) {
    ...
    // On period change I think we are going to use the wrong value on the first segment
    // init segment clears offset, but we don't see that yet, can we move inside initSourceBuffer?
  ...
  async initSourceBuffer_(mediaState, reference) {
    ...
    const appendWindowStart = Math.max(0,
        reference.appendWindowStart - this.config_.appendWindowStartAdjustment);
    const appendWindowEnd =
        reference.appendWindowEnd + this.config_.appendWindowEndAdjustment;
