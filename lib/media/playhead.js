...
shaka.media.MediaSourcePlayhead = class {
  ...
  constructor(mediaElement, manifest, config, startTime, onSeek, onEvent) {
    ...
    /** private {boolean} */
    this.pausedForLiveWindow_ = false;
  ...
  onPollWindow_() {
    ...
    if (this.mediaElement_.readyState == 0 ||
        (this.mediaElement_.paused && !this.pausedForLiveWindow_)) {
      return;
    }
    ...
    if (this.config_.pauseToGetBackInSeekRange) {
      if (currentTime > seekEnd) {
        if (!this.mediaElement_.paused) {
          shaka.log.info('Pausing because currentTime > seekEnd ' +
              '(currentTime - seekEnd) = ', (currentTime - seekEnd));
          this.pausedForLiveWindow_ = true;
          this.mediaElement_.pause();
        }
      } else if (this.mediaElement_.paused && this.pausedForLiveWindow_) {
        shaka.log.info('Resuming because currentTime < seekEnd ' +
            '(currentTime - seekEnd) = ', (currentTime - seekEnd));
        this.pausedForLiveWindow_ = false;
  ...
  onSeeking_() {
    ...
    const currentTime = this.videoWrapper_.getTime();
    // When seek is caused by a gap jump do not reposition, because
    // the gap jump may move the playhead past the end of the seek range
    // temporarily. Playback will be paused later to get the playhead
    // back into the seek range.
    const targetTime = (this.config_.pauseToGetBackInSeekRange &&
          this.gapController_.hasGapJumpOccurred()) ?
          currentTime :
          this.reposition_(currentTime);
