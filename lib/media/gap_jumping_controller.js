...
shaka.media.GapJumpingController = class {
  ...
  constructor(video, timeline, config, stallDetector, onEvent) {
    ...
    /** @private {boolean} */
    this.hasGapJumpOccurred_ = false;

    /** @private {?number} */
    this.lastGapJumpTo_ = null;
  ...
  onPollGapJump_() {
    ...
    const gapOverJump = this.config_.gapOverJump || 0;
    ...
    if (!this.config_.pauseToGetBackInSeekRange && jumpTo >= seekEnd) {
      return;
    }

    const gapStart = (gapIndex == 0) ? currentTime : buffered.end(gapIndex - 1);

    // Don't jump the same gap twice because this can cause looping
    if (jumpTo == this.lastGapJumpTo_) {
      return;
    }
    ...
    // We don't need to jump gaps that are smaller than the gap tolerance
    // because the browser is expected to play through these gaps.
    if ((jumpTo - gapStart) < this.config_.gapTolerance) {
      if (this.config_.reportGapJumps) {
        // Don't want to report same gap multiple times
        this.lastGapJumpTo_ = jumpTo;
        // We need to know about the gaps that we are not jumping incase they
        // are associated with subsequent stall.
        const gapJumpEvent = new shaka.util.FakeEvent('gapjump', new Map([
          ['currentTime', currentTime],
          ['preJump', (gapIndex == 0) ? 0 : gapStart - currentTime],
          ['gapSize', jumpTo - gapStart],
          ['gapStart', gapStart],
          ['gapEnd', jumpTo],
          ['overJump', gapOverJump],
          ['belowTolerance', true], // indidcates we are not jumping the gap
        ]));
        this.onEvent_(gapJumpEvent);
      }
      return;
    }
    ...
    if (isGapSmall || jumpLargeGap) {
      if (gapIndex == 0) {
        shaka.log.info(
            'Jumping forward', (jumpSize + gapOverJump),
      ...
      if (this.config_.reportGapJumps && !this.didFireLargeGap_) {
        const gapJumpEvent = new shaka.util.FakeEvent('gapjump', new Map([
          ['currentTime', currentTime],
          ['preJump', (gapIndex == 0) ? 0 : gapStart - currentTime],
          ['gapSize', jumpTo - gapStart],
      ...
      this.hasGapJumpOccurred_ = true;
      this.lastGapJumpTo_ = jumpTo;
      this.video_.currentTime = jumpTo + gapOverJump;
    }
  }

  /**
   * Determines if a gap jump has occurred since the last call to this function.
   * @return {boolean}
   */
  hasGapJumpOccurred() {
    const result = this.hasGapJumpOccurred_;
    this.hasGapJumpOccurred_ = false;
    return result;
  }
};
 ...
 * handle for us.
 *
 * For most purposes this value has been replaced by the configuration field
 * streaming.gapTolerance. We are keeping this constant for one usage that seems
 * to serve a slightly different purpose.
 * @const
 */
