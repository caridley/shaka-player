...
shaka.media.InitSegmentReference = class {
   ...
   */
  constructor(uris, startByte, endByte, mediaQuality = null, periodId = null) {
  ...
  /**
   * Returns DASH periodId
   *
   * @return {?string} the piereod id
   */
  getPeriodId() {
...
shaka.media.SegmentReference = class {
  ...
  constructor(
    ...
    /** @type {boolean} */
    this.timestampOffsetCorrected = false;
  }
  ...
  /**
   * Change the timestampOffset and note that it has been corrected.
   *
   * @param {number} timestampOffset
   */
  correctTimestampOffset(timestampOffset) {
    if (!this.timestampOffsetCorrected) {
      this.timestampOffset = timestampOffset;
      this.timestampOffsetCorrected = true;
    }
  }
};
