...
shaka.media.MediaSourceEngine = class {
  ...
  constructor(video, closedCaptionParser, textDisplayer,
    ...
    const parseInbandPsshNoOp = (mediaSegment) => {
      return Promise.resolve();
    };

    /** @private {!function(!BufferSource): Promise<void>} */
    this.parseInbandPssh_ = parseInbandPssh || parseInbandPsshNoOp;
  ...
  async appendBuffer(contentType, data, startTime, endTime, hasClosedCaptions) {
    ...
    if (contentType == ContentType.TEXT) {
      ...
      await this.parseInbandPssh_(data);
