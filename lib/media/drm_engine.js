...
shaka.media.DrmEngine = class {
  ...
  constructor(playerInterface, updateExpirationTime = 1) {
    ...
    this.newInitDataPromise = Promise.resolve();
  ...
  async newInitData(initDataType, initData) {
    // Wait for the previous license request to complete
    await this.waitForNewInitData_();
  ...
  async waitForNewInitData_() {
    ...
    try {
      ...
      shaka.log.warning('Timeout waiting for newInitData to compelete');
    }
  }
