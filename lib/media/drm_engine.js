...
shaka.media.DrmEngine = class {
   ...
   * @return {Promise<void>}
   */
  async newInitData(initDataType, initData) {
    // Suppress duplicate init data.
    // Note that some init data are extremely large and can't portably be used
    // as keys in a dictionary.

    shaka.log.info('wait for license start - in progress');
    if (this.newInitDataPromise) {
      await this.newInitDataPromise;
    }
    shaka.log.info('wait for license end - in progress');
    ...
    shaka.log.info('wait for license start');
    this.newInitDataPromise = new Promise(
        (resolve) => new shaka.util.Timer(resolve).tickAfter(2));

    await this.newInitDataPromise;
    shaka.log.info('wait for license end');
  }
  ...
  /**
   * Parse from a media segment and add the init data
   *
   * @param {!BufferSource} mediaSegment
   * @return {Promise<void>}
   */
  async parseInbandPssh(mediaSegment) {
    if (! this.config_.parseInbandPssh) {
      return;
    }
    ...
    if (totalLength > 0) {
      ...
      await this.newInitData('cenc', combinedData);
    }
  }
};
