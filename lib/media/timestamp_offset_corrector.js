...
shaka.media.TimestampOffsetCorrector = class {
  ...
  constructor() {
    ...
    /**
     * Maps a contentType, e.g., 'audio', 'video', or 'text' to 
     * a Map of trackNumber to timescale
     *
     * @private {!Map.<shaka.util.ManifestParserUtils.ContentType,
     *                 !Map<number, number>>}
     */
  ...
  parseBaseMediaDecodeTime_(contentType, segmentData) {
    ...
    const timescale = this.getTimescale_(contentType, trackId);
    return baseMediaDecodeTime / timescale;
  }

  /**
   * Get timescale for contentType / trackId using data parsed
   * from the init segment.
   * 
   * @param {shaka.util.ManifestParserUtils.ContentType} contentType
   * @param {number} trackId
   * @return {number} the timescale for the contentType / trackId
   * @private
   */
   getTimescale_(contentType, trackId) {
    let timescale = 1;
